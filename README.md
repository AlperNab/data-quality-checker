...
**Domain:** `Data Engineering` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Data Quality Checker** is a standalone, browser-based AI workflow app for **Data Engineering**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Dataset → quality report and fixes.
...
|---|---:|:---:|---|---|
| `dataset`<br>Dataset | text | Yes | — | Affects input: Dataset. |
...
| `thresholds`<br>thresholds | slider | No | 50 | Affects customization: thresholds. |
| `column_rules`<br>column rules | textarea | No | — | Affects customization: column rules. |
| `business_definitions`<br>business definitions | text | No | — | Affects customization: business definitions. |
| `export_framework`<br>export framework | select | No | auto detect | Affects customization: export framework. |
| `schedule`<br>schedule | text | No | — | Affects customization: schedule. |
| `severity_levels`<br>severity levels | select | No | — | Affects customization: severity levels. |
...
- **export framework**: auto detect, React, Next.js, Laravel, NestJS, .NET, Django, FastAPI, Custom
...
uvicorn app.main:app --host 127.0.0.1 --port 9115
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9115`. |
...
```bash
curl http://127.0.0.1:9115/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9115/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9115 uvicorn app.main:app --host 0.0.0.0 --port 9115
```
...
- `plugins/data-quality-checker.json` for plugin metadata.
