# Data Quality Checker — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9115`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/data-quality-checker.json` — this project’s own feature/customization/input schema.
...
- `thresholds` — thresholds (slider)
- `column_rules` — column rules (textarea)
- `business_definitions` — business definitions (text)
- `export_framework` — export framework (select)
- `schedule` — schedule (text)
- `severity_levels` — severity levels (select)
